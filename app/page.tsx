...
// Status of each module in the Next.js migration ("legacy" | "partial" |
// "migrated"). Update as more modules move over — "legacy" links to the old
// relative .html path and expects the old static site to still be reachable
// there; "partial" means only part of the module is ported (see that
// module's own page for what's still missing).
const MODULES = [
  {
    title: "Konversi",
      ...
      <div className="modules">
        {MODULES.map((m) => (
          <Link key={m.title} className="module" href={m.href}>
            ...
            <div className={`status live${m.status === "partial" ? " partial" : ""}`}>
              {m.status === "migrated"
                ? "Live (Next.js)"
                : m.status === "partial"
                  ? "Sebagian (Next.js)"
