...
// Palet warna tabel — disamain 1 sumber warna aja: ijo brand Samaya (di-sample
// langsung dari logo header, ≈ RGB 38/180/113), bukan navy (20,40,70) + biru
// terang (29,91,212) + border abu-kebiruan (slate-300) yang kepake terpisah
// sebelumnya. 3 warna beda yang gak nyambung ke identitas perusahaan itu yang
// bikin dokumennya kesan "template generik/dibikin robot", bukan surat resmi
// yang didesain sendiri sama orang. Sekarang SEMUA elemen tabel (header bar,
// total bar, link e-Katalog, garis grid, zebra-stripe) turunan dari satu
// warna brand yang sama, cuma beda gelap/terang — border & zebra dibikin
// versi TINT (dicampur putih) dari warna solid-nya, fill dibikin versi SHADE
// (digelapin dikit) biar teks putih di atasnya tetap kebaca jelas (kontras
// WCAG AA, ~5.3:1) — bukan asal comot warna cerah kayak sebelumnya.
const SPH_BRAND = [18, 116, 74];        // ijo tua brand — fill header/total bar
const SPH_LINK_COLOR = [29, 91, 212];    // biru — KHUSUS teks link "Lihat di e-Katalog" (bukan ijo)
const SPH_BORDER_COLOR = [30, 30, 30];   // garis grid tabel: item/gelap solid (bukan tint warna), biar tegas kayak tabel Word/Excel biasa
const SPH_ZEBRA_COLOR = [244, 250, 247];  // tint ijo brand paling muda, buat baris selang-seling
const SPH_BORDER_WIDTH = 0.2;
    ...
    if (btn && input) {
      ...
      input.addEventListener('change', async () => {
        ...
        try {
          ...
          S.showToast('Gagal memuat file: ' + err.message, 'error');
        }
      });
    }
    if (retryBtn) {
...
function sphOpenRecordModal() {
  ...
  rsphValue.textContent = S.rupiah(sphLastRecord.totalValue);
  rsphNamaFile.textContent = sphLastRecord.namaFile;
  rsphOwner.textContent = (typeof S.inpMarsup !== 'undefined' && S.inpMarsup.value.trim()) || '(belum diisi)';
  rsphLink.value = '';
...
async function sphGetLatestKonversiRecordId(sesiId) {
  ...
  try {
    const res = await S.sesiFetch(`konversi_record?sesi_id=eq.${sesiId}&select=id,revisi&order=revisi.desc&limit=1`);
...
async function sphGetNextRevisi(sesiId) {
  if (!sesiId) return 0;
  try {
    const res = await S.sesiFetch(`sph_records?sesi_id=eq.${sesiId}&select=revisi&order=revisi.desc&limit=1`);
    if (!res.ok) return 0;
    const rows = await res.json();
...
async function sphRecordToSupabase(record, ownerValue) {
  const sesiIdUntukSph = (typeof S.currentSesiId !== 'undefined') ? S.currentSesiId : null;
  ...
  const headerRes = await S.sesiFetch('sph_records', {
    method: 'POST',
  ...
  if (record.items && record.items.length) {
    ...
    const itemRes = await S.sesiFetch('sph_record_items', {
      method: 'POST',
...
recordSphSubmitBtn.addEventListener('click', async () => {
  ...
  if (!sphLastRecord) return;
  const ownerValue = (typeof S.inpMarsup !== 'undefined' && S.inpMarsup.value.trim()) || '';
  if (!ownerValue) {
  ...
  try {
    ...
    const res = await fetch(S.SHEETS_PROXY_URL, {
      method: 'POST',
      headers: {
...
function sphPrefillFromSession() {
  ...
  if (!sphRsInput.value && typeof S.inpRs !== 'undefined' && S.inpRs.value) sphRsInput.value = S.inpRs.value;
  if (!sphSalesInput.value && typeof S.inpSales !== 'undefined' && S.inpSales.value) sphSalesInput.value = S.inpSales.value;
  sphItemCountEl.textContent = String(S.clipboard.length);
}
...
function sphAddPageWithFooter(doc) {
  ...
  // Jarak logo->konten disamain 8mm (dulu 6mm, kerasa mepet/sumpek dibanding
  // halaman pertama yg jaraknya 8mm lewat Math.max(headerH,8)+6 di sphGenerate()
  // — sekarang dua-duanya sama-sama +8 biar konsisten di semua halaman).
  return SPH_MARGIN_TOP + (headerH ? headerH + 8 : 0);
}

function sphValidate() {
  const errs = [];
  if (!S.clipboard.length) errs.push('Belum ada produk di Clipboard.');
...
async function sphFetchThumbForKodes(kodeAsliByKode, onProgress) {
  ...
  await Promise.all(kodes.map(async (kode) => {
    const raw = await S.fetchImageBase64(kodeAsliByKode.get(kode), kode);
    if (raw) {
      const clean = await S.removeBackground(raw);
...
async function sphDrawLampiranTablePage(doc, title, tableItems, safeBottom) {
  ...
  function drawLampHeader(yy) {
    doc.setFillColor(...SPH_BRAND);
    doc.setDrawColor(...SPH_BORDER_COLOR);
    doc.setLineWidth(SPH_BORDER_WIDTH);
    doc.rect(SPH_MARGIN_X, yy, tableW, 8.5, 'FD');
    let hx = SPH_MARGIN_X;
    for (let ci = 0; ci < lWidths.length - 1; ci++) { hx += lWidths[ci]; doc.line(hx, yy, hx, yy + 8.5); }
  ...
  // Garis aksen tipis warna brand di bawah judul lampiran — biar section title
  // gak cuma teks polos ngambang, kesannya lebih "didesain" dikit (rapi).
  ly += 3;
  const titleW = doc.getTextWidth(title.toUpperCase());
  doc.setDrawColor(...SPH_BRAND);
  doc.setLineWidth(0.5);
  doc.line(SPH_PAGE_W / 2 - titleW / 2, ly, SPH_PAGE_W / 2 + titleW / 2, ly);
  ly += 6;
  ...
  for (let idx = 0; idx < tableItems.length; idx++) {
    ...
    if (idx % 2 === 0) { doc.setFillColor(...SPH_ZEBRA_COLOR); doc.rect(SPH_MARGIN_X, ly, tableW, rowH, 'F'); }
    doc.setDrawColor(...SPH_BORDER_COLOR);
    doc.setLineWidth(SPH_BORDER_WIDTH);
    doc.rect(SPH_MARGIN_X, ly, tableW, rowH, 'S');
    let lgx = SPH_MARGIN_X;
    for (let ci = 0; ci < lWidths.length - 1; ci++) { lgx += lWidths[ci]; doc.line(lgx, ly, lgx, ly + rowH); }
  ...
  doc.setDrawColor(...SPH_BORDER_COLOR);
  doc.setLineWidth(SPH_BORDER_WIDTH);
  doc.rect(SPH_MARGIN_X, ly, tableW, 8.5, 'FD');
  doc.setTextColor(255, 255, 255);
  doc.setFontSize(SPH_FONT_SIZE);
...
async function sphDrawLampiranUnitPages(doc, title, pages, safeBottom) {
  ...
  for (let idx = 0; idx < pages.length; idx++) {
    ...
    doc.setFont('helvetica', 'normal');
    ly += 3;
    const fullTitleW = doc.getTextWidth(`LAMPIRAN — ${title}`);
    doc.setDrawColor(...SPH_BRAND);
    doc.setLineWidth(0.5);
    doc.line(SPH_PAGE_W / 2 - fullTitleW / 2, ly, SPH_PAGE_W / 2 + fullTitleW / 2, ly);
    ly += 6;
...
function sphBuildDocxBlob(items, meta) {
  ...
  const bodyRows = items.map((it, idx) => {
    ...
    const cells = [
      ...
      bodyCell(S.rupiah(it.harga), widthsPct[4], { align: AlignmentType.RIGHT, shade }),
      bodyCell(S.rupiah(it.total), widthsPct[5], { align: AlignmentType.RIGHT, shade }),
    ];
  ...
  const totalRow = new TableRow({
    children: [
      ...
      new TableCell({
        ...
        shading: { fill: '1D5BD4' },
        borders,
        children: [new Paragraph({ alignment: AlignmentType.RIGHT, children: [new TextRun({ text: S.rupiah(meta.grandTotal), bold: true, color: 'FFFFFF', size: 18 })] })],
      }),
    ],
  });
...
async function sphGenerate() {
  ...
  try {
    ...
    let no = 0;
    const items = S.clipboard.map(item => {
      no++;
      const harga = S.modeSwastaOutput ? (item.harga_swasta || 0) : (item.harga_ekat || 0);
    ...
    const dateY = y + Math.max(headerH, 8) + 8;
    ...
    const headers = S.modeSwastaOutput
      ? ['No', 'Kode', 'Deskripsi', 'Qty', 'Harga', 'Total']
      : ['No', 'Kode', 'Deskripsi', 'Qty', 'Harga', 'Total', 'E-Katalog v6'];
    // Lebar kolom Harga/Total DIHITUNG DINAMIS dari angka terpanjang yang
    // beneran ada di data (termasuk grandTotal), bukan angka tebakan tetap
    // lagi. Sebelumnya sempet dilebarin manual ke 30/33mm — cukup buat
    // ratusan juta, tapi begitu ada barang senilai miliaran ("Rp
    // 6.005.855.244", ~30.6mm) tetep aja mepet/nabrak kolom Qty lagi.
    // Sekarang lebar kolom ngikutin isi apa adanya, jadi berapa pun besar
    // angkanya (juta/miliar/triliun) kolomnya otomatis muat + jarak aman.
    // Kolom Harga & Total dibikin SAMA LEBAR (dihitung dari yang paling
    // lebar di antara kedua kolom itu di SEMUA baris + grandTotal) — Total
    // bisa aja lebih panjang dari Harga (kalau qty > 1 nambah 1 digit), jadi
    // paling aman disamain daripada ditebak salah satu lebih pendek.
    //
    // Sisa lebar tabel (di luar No/Qty/E-Katalog yg tetep, & Harga/Total yg
    // baru dihitung) dibagi ke Kode & Deskripsi, PAKAI PROPORSI TETAP kayak
    // sebelumnya (24mm Kode, sisanya Desc) selama masih muat. Kalau
    // angkanya sampai SEGITU gede (miliaran ganda/triliun) sampai jatah
    // Kode+Desc kepepet, Kode baru ikutan dikecilin (lantai 16mm — kode
    // produk emang udah pasti wrap jadi 2-3 baris pendek gitu, ga masalah)
    // biar Deskripsi tetep dapet lantai minimum 22mm dan total lebar kolom
    // selalu PAS SAMA DENGAN lebar tabel (gak meleset render grid-nya).
    doc.setFontSize(SPH_FONT_SIZE);
    const amountTexts = items.flatMap(it => [S.rupiah(it.harga), S.rupiah(it.total)]).concat([S.rupiah(grandTotal)]);
    const maxAmountTextW = Math.max(...amountTexts.map(t => doc.getTextWidth(t)));
    // +1.5mm padding kanan (posisi rata-kanan teks) + 1.5mm jarak aman ke
    // garis pembatas kolom sebelumnya + 1mm buffer ekstra.
    const amountColW = Math.max(26, Math.ceil(maxAmountTextW + 4));
    const noW = 9, qtyW = 11, katalogW = 32;
    const fixedW = S.modeSwastaOutput ? noW + qtyW : noW + qtyW + katalogW;
    const kodeDescBudget = tableW - fixedW - amountColW * 2;
    const kodeW = Math.min(24, Math.max(16, kodeDescBudget - 22));
    const descW = kodeDescBudget - kodeW;
    const widths = S.modeSwastaOutput
      ? [noW, kodeW, descW, qtyW, amountColW, amountColW]
      : [noW, kodeW, descW, qtyW, amountColW, amountColW, katalogW];
    function drawTableHeader(yy) {
      doc.setFillColor(...SPH_BRAND);
      doc.setDrawColor(...SPH_BORDER_COLOR);
      doc.setLineWidth(SPH_BORDER_WIDTH);
      doc.rect(SPH_MARGIN_X, yy, tableW, 8.5, 'FD');
      // Garis vertikal antar kolom di dalam header — outer border-nya udah
      // kepegang sama rect('FD') di atas, ini cuma pemisah internal.
      let hx = SPH_MARGIN_X;
      for (let ci = 0; ci < widths.length - 1; ci++) { hx += widths[ci]; doc.line(hx, yy, hx, yy + 8.5); }
      doc.setTextColor(255, 255, 255);
      doc.setFontSize(SPH_FONT_SIZE);
      let x = SPH_MARGIN_X;
    ...
    items.forEach((item, idx) => {
      ...
      if (idx % 2 === 0) { doc.setFillColor(...SPH_ZEBRA_COLOR); doc.rect(SPH_MARGIN_X, y, tableW, rowH, 'F'); }
      // Grid baris: border penuh (atas/bawah/kiri/kanan) + pemisah kolom
      // internal. Ditumpuk di ATAS fill zebra tapi SEBELUM teks, biar garis
      // gak numpuk nutupin isi sel.
      doc.setDrawColor(...SPH_BORDER_COLOR);
      doc.setLineWidth(SPH_BORDER_WIDTH);
      doc.rect(SPH_MARGIN_X, y, tableW, rowH, 'S');
      let gx = SPH_MARGIN_X;
      for (let ci = 0; ci < widths.length - 1; ci++) { gx += widths[ci]; doc.line(gx, y, gx, y + rowH); }
      doc.setFontSize(SPH_FONT_SIZE);
      ...
      for (let li = 0; li < lineCount; li++) {
        ...
        x += widths[3];
        if (li === 0) doc.text(S.rupiah(item.harga), x + widths[4] - 1.5, ly2, { align: 'right' });
        x += widths[4];
        if (li === 0) doc.text(S.rupiah(item.total), x + widths[5] - 1.5, ly2, { align: 'right' });
        x += widths[5];
        if (li === 0 && !S.modeSwastaOutput) {
          if (item.link) {
            doc.setTextColor(...SPH_LINK_COLOR);
    ...
    doc.setDrawColor(...SPH_BORDER_COLOR);
    doc.setLineWidth(SPH_BORDER_WIDTH);
    doc.rect(SPH_MARGIN_X, y, tableW, 8.5, 'FD');
    doc.setTextColor(255, 255, 255);
    doc.setFontSize(SPH_FONT_SIZE);
    doc.text('TOTAL VALUE', SPH_MARGIN_X + widths[0] + widths[1] + widths[2] - 1.5, y + 5.8, { align: 'right' });
    doc.text(S.rupiah(grandTotal), SPH_MARGIN_X + widths[0] + widths[1] + widths[2] + widths[3] + widths[4] + widths[5] - 1.5, y + 5.8, { align: 'right' });
    doc.setTextColor(17, 24, 39);
    y += 15;
    ...
    if (sphIncludeLampiranCheckbox && sphIncludeLampiranCheckbox.checked && S.clipboard.length) {
      ...
      if (sets.length) {
        ...
        for (let s = 0; s < sets.length; s++) {
          setItemsMap[sets[s].kode_produk] = await S.getSetItems(sets[s].kode_produk);
      ...
      for (let u = 0; u < S.clipboard.length; u++) {
        const item = S.clipboard[u];
        sphStatusEl.textContent = `Mengecek lampiran brosur… (${u + 1}/${S.clipboard.length})`;
        const { pages } = await S.getLampiranPagesForKode(item.kode_produk);
    ...
    try {
      const docxBlob = await sphBuildDocxBlob(items, { nomorSph, namaRs, tanggalSurat, grandTotal, modeSwastaOutput: S.modeSwastaOutput });
    ...
    sphStatusEl.textContent = `Siap — ${items.length} produk, total ${S.rupiah(grandTotal)}.`;
    ...
    S.showToast('Gagal generate SPH: ' + err.message, 'error');
  } finally {
...
[sphNomorInput, sphRsInput].forEach(el => {
  ...
  // Inline handler yang masih ada di markup (cuma satu: mobile tab switcher —
  // lihat map.md/investigasi sesi ini, sisanya sudah pakai addEventListener).
  window.switchTab = S.switchTab;

  // UPDATE (sesi lanjutan -- nav redesign): initial sub-tab, driven by the
  // URL's sub-route if the user arrived via a deep link (global sidebar's
  // accordion, or a bookmark/refresh on e.g. #konversian/riwayat) --
  // defaults to 'cari' same as before this session (previously implicit,
  // markup.js's static HTML just happened to start with subtab-cari
  // active/panel-body-cari visible; explicit now since a direct link to a
  // different sub-tab needs this to actually land there). konversian's own
  // in-page subtab row (subtabCari.addEventListener(...) etc, right above
  // switchSubTab's definition) is UNCHANGED and still works exactly as
  // before -- this is purely an ADDITIONAL way to land on a given tab, not
  // a replacement for the existing one. See subnav.js's header comment for
  // why that row isn't being removed this session.
  S.switchSubTab(S.initialSub || 'cari');
}