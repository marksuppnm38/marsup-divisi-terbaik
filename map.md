...
3. ✅ **`konversian.html`** — SELESAI dimigrasi (22 Agustus 2026). Paling gede tapi paling siap secara auth (satu-satunya yang udah PNMAuth penuh). Kerjaannya di sini murni soal pisah-pisah section jadi modul (lihat bagian 2), bukan benerin auth.
4. ✅ **`crud-produk.html`** — SELESAI dimigrasi (23 Agustus 2026, sesi kesebelas). Auth manual (`sb.auth.signInWithPassword/signOut/onAuthStateChange`) diganti penuh ke `PNMAuth.login/logout/onAuthStateChange` DULU, baru dibungkus SPA sekaligus dalam sesi yang sama — lihat detail di Status di bawah.
